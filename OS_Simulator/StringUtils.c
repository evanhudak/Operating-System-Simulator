...
void concatenateString(char *destStr, const char *sourceStr)
{
	...
	// copy source string in case of aliasing
	    ...
	    tempStr = (char *)malloc((sourceStrLen + 1) * sizeof(char));
	                      ...
	                      char *capturedString)
{
	...
	while(intChar != EOF
		    && ((clearLeadingNonPrintable && intChar < (int)SPACE_CHAR)
		      || (clearLeadingSpace && intChar == (int)SPACE_CHAR)))
    {
	    // get next character
	        // function: fgetc
    	intChar = fgetc(inStream);
    ...
    while(
	      ...
	      && ((stopAtNonPrintable && intChar >= (int)SPACE_CHAR)
	      || (!stopAtNonPrintable))
